...
class SignupForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if email:
            ...
            if qs.count() == 0:
                raise ValidationError("Your email adress is not allowed")
    ...
    def send_verification_email(self, user):
        ...
        verification_url = (
            f"{settings.SITE_URL}{reverse('verify_email', args=[token.token])}"
        )
        ...
        requests.post("https://api.eu.mailgun.net/v3/mail.carvsn.com/messages",
  		auth=("api", settings.EMAIL_API_KEY),
  		data={"from": f"Visual Analytics <{settings.DEFAULT_FROM_EMAIL}>",
			"to": f"<{user.email}>",
